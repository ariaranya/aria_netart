...
现在，当调整窗口大小时，画布的大小将根据窗口的宽度和高度进行自适应，并且图形的大小将保持不变。
...
User
当我把canvas 调整成widowWidth windowHeight 之后 可以让现在的图形显示在屏幕正中，并保持现在的大小 不要被拉升放大吗
ChatGPT
如果您希望将画布调整为窗口的宽度和高度，并使现有的图形显示在屏幕正中，并且保持当前大小而不被拉伸放大，您可以采取以下步骤：

在setup()函数中，使用createCanvas(windowWidth, windowHeight);将画布设置为窗口的宽度和高度。
在draw()函数中，使用translate()函数将坐标原点移到屏幕中心。
在draw()函数中，使用scale()函数将图形按比例缩小，以保持当前大小。缩放因子可以通过调整scaleFactor变量来控制。
以下是经过修改的代码：
